...
FOR %%x IN ("c:\wamp\www\ols.astem.local\application\library\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=c:\wamp\www\ols.astem.local\application\library\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=c:\wamp\www\ols.astem.local\var\pear"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=C:\wamp\bin\php\php7.1.9\php.exe"
SET "PHP_PEAR_SYSCONF_DIR=c:\wamp\www\ols.astem.local\application\library\pear"
