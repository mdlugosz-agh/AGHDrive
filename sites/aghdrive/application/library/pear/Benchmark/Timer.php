...
class Benchmark_Timer extends PEAR
{
     ...
     * @access public
     */
    function timeElapsed($start = 'Start', $end = 'Stop') : float
    {
        ...
        if (extension_loaded('bcmath')) {
        	return (float)(bcsub($end, $start, 6));
        } else {
    ...
    public function getProfiling() : array
    {
        ...
        foreach ($this->markers as $marker => $time) {
            if (extension_loaded('bcmath')) {
            	$diff  = (float)bcsub($time, $temp, 6);
            	$total = (float)bcadd($total, $diff, 6);
            } else {
            	$diff  = (float)($time - $temp);
            	$total = (float)($total + $diff);
            }
