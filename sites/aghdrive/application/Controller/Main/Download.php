...
class Controller_Main_Download extends Controller
{
	...
	public function list() : Data_Response
	{
		...
		// Read download list
        $this->response->download_list = $this->readContentList(PATH_TO_CONTENT_DOWNLOAD);
		
		$this->response->template = 'download/list';
	...
	public function info()
	{
		...
		// Read content of download info
        $this->response->download_info = $this->readContent(PATH_TO_CONTENT_DOWNLOAD, $this->request->route['id'] . '.php');
        
		$this->response->template = 'download/info';
	 ...
	 */
	public function download() : Data_Response
	{
		parent::run();
	
		$file = PATH_TO_DATA . DIRECTORY_SEPARATOR . str_replace('#', '/', urldecode($this->request->route['file']));
	 ...
	 * @return void
	 */
	private function _download($filePath) 
	{     
		if(!empty($filePath)) { 
